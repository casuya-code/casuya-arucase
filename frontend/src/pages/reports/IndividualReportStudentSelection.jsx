...
const IndividualReportStudentSelection = () => {
  ...
  const { data: students = [], isLoading, error } = useQuery({
    queryKey: ['report-students', form, stream, year],
    queryFn: async () => {
  ...
  const handleStudentClick = (admNo) => {
    ...
    navigate(`/reports/individual/${form}/${stream}/${year}/${term}/${admNo}`);
  };
    ...
    <AdminLayout>
      <div className="individual-report-page">
        <div className="breadcrumb">
          ...
          <Link to={`/reports/individual/${form}/${stream}/${year}/term`}>{year}</Link> &gt; {term}
        </div>
