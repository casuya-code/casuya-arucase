...
import { admissionKey, resultsByAdmissionNumber } from '../../services/preFormOneApiHelpers';
  ...
  const generateStudentPDF = async (student) => {
    ...
    try {
      const response = await fetch(
        buildFetchUrl(`/pre-form-one/${year}/continuing-results/${student.id}/pdf`),
        {
          headers: {
      ...
      toast.success(
        `${student.first_name} ${student.surname}'s continuing report downloaded successfully!`
      );
    } catch (error) {
      toast.error(
        error.message ||
          `Failed to generate continuing report for ${student.first_name} ${student.surname}`
      );
    } finally {
            ...
            {studentsLoading || resultsLoading ? (
              ...
              <>
                <div className="results-info">
                  ...
                  <div className="info-item">
                    <strong>Reports Available:</strong> {Object.keys(continuingResults).length}
                  </div>
                </div>
                        ...
                        <div key={student.id} className="student-report-card">
                          <div className="student-info">
                            <div className="student-details">
                              ...
                              <div className="student-meta">
                                ...
                                {student.parish && (
                                  <span className="parish">Parish: {student.parish}</span>
                                )}
                              </div>
                            </div>
                          ...
                          <div className="student-actions">
                            <button
                              ...
                              title={
                                hasReport
                                  ? `Generate PDF continuing report for ${student.first_name} ${student.surname}`
                                  : 'No continuing report data available'
                              }
                            >
