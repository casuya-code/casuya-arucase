...
const PreFormOneInterviewResults = () => {
  ...
  const { data: subjects = [], isLoading: subjectsLoading } = useQuery({
    queryKey: ['preform-one-interview-subjects', reportYear],
    queryFn: async () => {
      try {
        const list = await preFormOneInterviewSubjectsService.getSubjects();
  ...
  const { data: existingResults = {}, isLoading: resultsLoading } = useQuery({
    queryKey: ['preform-one-interview-results', reportYear, filter.month],
    queryFn: async () => {
      try {
        const res = await preFormOneService.getInterviewResults(reportYear, filter.month);
        if (
          res?.data?.results &&
  ...
  const calculateResultsMutation = useMutation({
    ...
    onSuccess: (response) => {
      toast.success(
        response?.message || response?.data?.message || 'Interview results calculated and saved.'
      );
  ...
  const downloadPDF = async () => {
    const btn = document.getElementById('downloadInterviewResultsBtn');
    const btnText = document.getElementById('downloadInterviewBtnText');
    if (btn && btnText) {
      btn.disabled = true;
    ...
    try {
      const response = await preFormOneService.downloadInterviewResultsPDF(reportYear);
      ...
      link.href = url;
      link.download = `PreFormOne_Interview_Results_${reportYear}.pdf`;
      link.click();
      ...
      if (btn && btnText) {
        btn.disabled = false;
        btnText.textContent = 'Download Interview Results (PDF)';
      }
    }
  };
  ...
  const handleDownloadCSV = () => {
    ...
    csv.push([]);
    csv.push([`PRE-FORM ONE INTERVIEW RESULTS ${reportYear}`]);
    csv.push([]);
    ...
    link.download = `PreFormOne_Interview_Results_${reportYear}.csv`;
    link.click();
          ...
          <div className="excel-card-body">
            {isLoading ? (
              ...
              <div className="empty-state">
                ...
                <h3>No Interview Subjects</h3>
                <p>Add interview subjects before viewing results.</p>
              </div>
            ) : (
          ...
          <>
            <div className="print-button-container">
              <button
                type="button"
                onClick={downloadPDF}
                id="downloadInterviewResultsBtn"
                className="download-btn-monthly"
              >
                <i className="fas fa-file-pdf"></i>{' '}
                <span id="downloadInterviewBtnText">Download Interview Results (PDF)</span>
              </button>
              <button
                type="button"
            ...
            <div className="report-header-section">
              ...
              <div className="test-info-bar">
                PRE-FORM ONE INTERVIEW RESULTS {reportYear}
