...
async function populateInterviewSubjectsCorrect() {
  try {
    console.log('🔍 INTERVIEW CORRECT: Populating all interview subjects with sample data...');
    ...
    console.log(`🔍 INTERVIEW CORRECT: Found ${students.rowCount} students and ${interviewSubjects.rowCount} interview subjects`);
    
    // Populate interview scores for all students and all subjects
    console.log('🔍 INTERVIEW CORRECT: Creating interview scores...');
    for (const student of students.rows) {
      console.log(`🔍 INTERVIEW CORRECT: Creating scores for ${student.admission_number} - ${student.first_name} ${student.surname}`);
    ...
    console.log('🔍 INTERVIEW CORRECT: Calculating interview results...');
    ...
    for (const result of interviewResults.rows) {
      ...
      console.log(`🔍 INTERVIEW CORRECT: Interview result for ${result.admission_number} - Position: ${position}, Grade: ${grade}, Average: ${result.average.toFixed(2)}`);
      position++;
    }
    
    console.log('✅ INTERVIEW CORRECT: All interview subjects populated successfully!');
    console.log(`🔍 INTERVIEW CORRECT: Summary:`);
    ...
    console.error('🔍 INTERVIEW CORRECT: Error populating data:', error);
    throw error;
  }
}

populateInterviewSubjectsCorrect()
  .then(() => {
    console.log('✅ INTERVIEW CORRECT: All interview subjects population completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ INTERVIEW CORRECT: All interview subjects population failed:', error);
    process.exit(1);
  });
