...
async function populateInterviewSubjectsSuccess() {
  try {
    console.log('🔍 INTERVIEW SUCCESS: Populating all interview subjects with sample data...');
    ...
    console.log(`🔍 INTERVIEW SUCCESS: Found ${students.rowCount} students and ${interviewSubjects.rowCount} interview subjects`);
    
    // Populate interview scores for all students and all subjects
    console.log('🔍 INTERVIEW SUCCESS: Creating interview scores...');
    for (const student of students.rows) {
      console.log(`🔍 INTERVIEW SUCCESS: Creating scores for ${student.admission_number} - ${student.first_name} ${student.surname}`);
    ...
    console.log('🔍 INTERVIEW SUCCESS: Calculating interview results...');
    ...
    for (const result of interviewResults.rows) {
      ...
      console.log(`🔍 INTERVIEW SUCCESS: Interview result for ${result.admission_number} - Position: ${position}, Grade: ${grade}, Average: ${result.average.toFixed(2)}`);
      position++;
    }
    
    console.log('✅ INTERVIEW SUCCESS: All interview subjects populated successfully!');
    console.log(`🔍 INTERVIEW SUCCESS: Summary:`);
    ...
    console.error('🔍 INTERVIEW SUCCESS: Error populating data:', error);
    throw error;
  }
}

populateInterviewSubjectsSuccess()
  .then(() => {
    console.log('✅ INTERVIEW SUCCESS: All interview subjects population completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ INTERVIEW SUCCESS: All interview subjects population failed:', error);
    process.exit(1);
  });
