...
async function populateInterviewSubjectsFinalCorrect() {
  try {
    console.log('🔍 INTERVIEW FINAL CORRECT: Populating all interview subjects with sample data...');
    ...
    console.log(`🔍 INTERVIEW FINAL CORRECT: Found ${students.rowCount} students and ${interviewSubjects.rowCount} interview subjects`);
    
    // Populate interview scores for all students and all subjects
    console.log('🔍 INTERVIEW FINAL CORRECT: Creating interview scores...');
    for (const student of students.rows) {
      console.log(`🔍 INTERVIEW FINAL CORRECT: Creating scores for ${student.admission_number} - ${student.first_name} ${student.surname}`);
    ...
    console.log('🔍 INTERVIEW FINAL CORRECT: Calculating interview results...');
    ...
    for (const result of interviewResults.rows) {
      ...
      // Fixed INSERT statement - matching columns to values
      await query(`
      ...
      console.log(`🔍 INTERVIEW FINAL CORRECT: Interview result for ${result.admission_number} - Position: ${position}, Grade: ${grade}, Average: ${result.average.toFixed(2)}`);
      position++;
    }
    
    console.log('✅ INTERVIEW FINAL CORRECT: All interview subjects populated successfully!');
    console.log(`🔍 INTERVIEW FINAL CORRECT: Summary:`);
    ...
    console.error('🔍 INTERVIEW FINAL CORRECT: Error populating data:', error);
    throw error;
  }
}

populateInterviewSubjectsFinalCorrect()
  .then(() => {
    console.log('✅ INTERVIEW FINAL CORRECT: All interview subjects population completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ INTERVIEW FINAL CORRECT: All interview subjects population failed:', error);
    process.exit(1);
  });
