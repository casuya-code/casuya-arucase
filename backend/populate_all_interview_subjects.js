...
async function populateAllInterviewSubjects() {
  try {
    console.log('🔍 INTERVIEW SUBJECTS: Populating all interview subjects with sample data...');
    ...
    console.log(`🔍 INTERVIEW SUBJECTS: Found ${students.rowCount} students and ${interviewSubjects.rowCount} interview subjects`);
    
    // Populate interview scores for all students and all subjects
    console.log('🔍 INTERVIEW SUBJECTS: Creating interview scores...');
    for (const student of students.rows) {
      console.log(`🔍 INTERVIEW SUBJECTS: Creating scores for ${student.admission_number} - ${student.first_name} ${student.surname}`);
    ...
    console.log('🔍 INTERVIEW SUBJECTS: Calculating interview results...');
    ...
    for (const result of interviewResults.rows) {
      ...
      console.log(`🔍 INTERVIEW SUBJECTS: Interview result for ${result.admission_number} - Position: ${position}, Grade: ${grade}, Average: ${result.average.toFixed(2)}`);
      position++;
    }
    
    console.log('✅ INTERVIEW SUBJECTS: All interview subjects populated successfully!');
    console.log(`🔍 INTERVIEW SUBJECTS: Summary:`);
    ...
    console.error('🔍 INTERVIEW SUBJECTS: Error populating data:', error);
    throw error;
  }
}

populateAllInterviewSubjects()
  .then(() => {
    console.log('✅ INTERVIEW SUBJECTS: All interview subjects population completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ INTERVIEW SUBJECTS: All interview subjects population failed:', error);
    process.exit(1);
  });
