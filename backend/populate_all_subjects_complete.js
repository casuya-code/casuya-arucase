...
async function populateAllSubjectsComplete() {
  try {
    console.log('🔍 COMPLETE: Populating ALL PreFormOne subjects with sample data...');
    ...
    console.log(`🔍 COMPLETE: Found ${students.rowCount} students, ${interviewSubjects.rowCount} interview subjects, ${continuingSubjects.rowCount} continuing subjects`);
    
    // Populate interview scores for all students
    console.log('🔍 COMPLETE: Creating interview scores...');
    for (const student of students.rows) {
      console.log(`🔍 COMPLETE: Interview scores for ${student.admission_number} - ${student.first_name} ${student.surname}`);
      
      for (const subject of interviewSubjects.rows) {
        let score;
        switch (subject.subject_code) {
          case 'MATH': score = Math.floor(Math.random() * 20) + 75; break; // 75-95
          case 'ENG': score = Math.floor(Math.random() * 15) + 80; break; // 80-95
          case 'SCI': score = Math.floor(Math.random() * 25) + 70; break; // 70-95
          case 'SST': score = Math.floor(Math.random() * 20) + 75; break; // 75-95
          case 'CIV': score = Math.floor(Math.random() * 15) + 80; break; // 80-95
          case 'RE': score = Math.floor(Math.random() * 20) + 75; break; // 75-95
          case 'GK': score = Math.floor(Math.random() * 10) + 85; break; // 85-95
          default: score = Math.floor(Math.random() * 30) + 65; // 65-95
        }
    ...
    // Populate continuing scores for all students
    console.log('🔍 COMPLETE: Creating continuing scores...');
    for (const student of students.rows) {
      console.log(`🔍 COMPLETE: Continuing scores for ${student.admission_number}`);
      
      for (const subject of continuingSubjects.rows) {
        let score;
        switch (subject.subject_code) {
          case 'MATH': score = Math.floor(Math.random() * 15) + 80; break; // 80-95
          case 'ENG': score = Math.floor(Math.random() * 10) + 85; break; // 85-95
          case 'SCI': score = Math.floor(Math.random() * 20) + 75; break; // 75-95
          case 'SST': score = Math.floor(Math.random() * 15) + 80; break; // 80-95
          case 'BIO': score = Math.floor(Math.random() * 15) + 80; break; // 80-95
          case 'CRE': score = Math.floor(Math.random() * 20) + 75; break; // 75-95
          case 'AGRI': score = Math.floor(Math.random() * 25) + 70; break; // 70-95
          case 'PE': score = Math.floor(Math.random() * 10) + 85; break; // 85-95
          default: score = Math.floor(Math.random() * 30) + 65; // 65-95
        }
    ...
    for (const result of interviewResults.rows) {
      ...
      console.log(`🔍 COMPLETE: Interview result for ${result.admission_number} - Position: ${position}, Grade: ${grade}`);
      position++;
    }
    
    // Calculate continuing results
    console.log('🔍 COMPLETE: Calculating continuing results...');
    ...
    for (const result of continuingResults.rows) {
      ...
      console.log(`🔍 COMPLETE: Continuing result for ${result.admission_number} - Position: ${position}, Grade: ${grade}`);
      position++;
    }
    
    console.log('✅ COMPLETE: ALL PreFormOne subjects populated successfully!');
...
populateAllSubjectsComplete()
  .then(() => {
    console.log('✅ COMPLETE: All PreFormOne data population completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ COMPLETE: All PreFormOne data population failed:', error);
    process.exit(1);
  });
