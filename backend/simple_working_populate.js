...
async function simpleWorkingPopulate() {
  try {
    console.log('🔍 SIMPLE: Simple working population...');
    ...
    const students = await query('SELECT id, admission_number, first_name, surname FROM preform_one_students WHERE year = $1 ORDER BY admission_number LIMIT 3', [year]);
    ...
    console.log(`🔍 SIMPLE: Processing ${students.rowCount} students with ${subjects.rowCount} subjects`);
    ...
    for (const result of interviewResults.rows) {
      ...
      console.log(`🔍 SIMPLE: Interview result for ${result.admission_number} - Position: ${position}, Grade: ${grade}`);
      position++;
    }
    
    console.log('✅ SIMPLE: PreFormOne interview data populated successfully!');
    console.log(`🔍 SIMPLE: Interview results: ${interviewResults.rowCount} students positioned`);
    console.log(`🔍 SIMPLE: PDF Generation: READY`);
    
  } catch (error) {
    console.error('🔍 SIMPLE: Error populating data:', error);
    throw error;
  }
}

simpleWorkingPopulate()
  .then(() => {
