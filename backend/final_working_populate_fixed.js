...
async function finalWorkingPopulate() {
  try {
    console.log('🔍 FINAL WORKING: Final working population...');
    ...
    console.log(`🔍 FINAL WORKING: Processing ${students.rowCount} students with ${subjects.rowCount} subjects`);
    ...
    for (const result of interviewResults.rows) {
      ...
      console.log(`🔍 FINAL WORKING: Interview result for ${result.admission_number} - Position: ${position}, Grade: ${grade}`);
      position++;
    }
    
    console.log('✅ FINAL WORKING: PreFormOne interview data populated successfully!');
    console.log(`🔍 FINAL WORKING: Interview results: ${interviewResults.rowCount} students positioned`);
    console.log(`🔍 FINAL WORKING: PDF Generation: READY`);
    
  } catch (error) {
    console.error('🔍 FINAL WORKING: Error populating data:', error);
    throw error;
  }
}
...
finalWorkingPopulate()
  .then(() => {
    console.log('✅ FINAL WORKING: Data population completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ FINAL WORKING: Data population failed:', error);
    process.exit(1);
  });
