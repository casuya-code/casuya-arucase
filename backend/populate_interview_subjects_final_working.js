...
async function populateInterviewSubjectsFinalWorking() {
  try {
    console.log('🔍 INTERVIEW FINAL WORKING: Populating all interview subjects with sample data...');
    ...
    console.log(`🔍 INTERVIEW FINAL WORKING: Found ${students.rowCount} students and ${interviewSubjects.rowCount} interview subjects`);
    
    // Populate interview scores for all students and all subjects
    console.log('🔍 INTERVIEW FINAL WORKING: Creating interview scores...');
    for (const student of students.rows) {
      console.log(`🔍 INTERVIEW FINAL WORKING: Creating scores for ${student.admission_number} - ${student.first_name} ${student.surname}`);
    ...
    console.log('🔍 INTERVIEW FINAL WORKING: Calculating interview results...');
    ...
    for (const result of interviewResults.rows) {
      ...
      console.log(`🔍 INTERVIEW FINAL WORKING: Interview result for ${result.admission_number} - Position: ${position}, Grade: ${grade}, Average: ${result.average.toFixed(2)}`);
      position++;
    }
    
    console.log('✅ INTERVIEW FINAL WORKING: All interview subjects populated successfully!');
    console.log(`🔍 INTERVIEW FINAL WORKING: Summary:`);
    ...
    console.error('🔍 INTERVIEW FINAL WORKING: Error populating data:', error);
    throw error;
  }
}

populateInterviewSubjectsFinalWorking()
  .then(() => {
    console.log('✅ INTERVIEW FINAL WORKING: All interview subjects population completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ INTERVIEW FINAL WORKING: All interview subjects population failed:', error);
    process.exit(1);
  });
