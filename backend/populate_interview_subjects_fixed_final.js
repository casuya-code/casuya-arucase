...
async function populateInterviewSubjectsFixedFinal() {
  try {
    console.log('🔍 INTERVIEW FIXED FINAL: Populating all interview subjects with sample data...');
    ...
    console.log(`🔍 INTERVIEW FIXED FINAL: Found ${students.rowCount} students and ${interviewSubjects.rowCount} interview subjects`);
    
    // Populate interview scores for all students and all subjects
    console.log('🔍 INTERVIEW FIXED FINAL: Creating interview scores...');
    for (const student of students.rows) {
      console.log(`🔍 INTERVIEW FIXED FINAL: Creating scores for ${student.admission_number} - ${student.first_name} ${student.surname}`);
    ...
    console.log('🔍 INTERVIEW FIXED FINAL: Calculating interview results...');
    ...
    for (const result of interviewResults.rows) {
      ...
      console.log(`🔍 INTERVIEW FIXED FINAL: Interview result for ${result.admission_number} - Position: ${position}, Grade: ${grade}, Average: ${result.average.toFixed(2)}`);
      position++;
    }
    
    console.log('✅ INTERVIEW FIXED FINAL: All interview subjects populated successfully!');
    console.log(`🔍 INTERVIEW FIXED FINAL: Summary:`);
    ...
    console.error('🔍 INTERVIEW FIXED FINAL: Error populating data:', error);
    throw error;
  }
}

populateInterviewSubjectsFixedFinal()
  .then(() => {
    console.log('✅ INTERVIEW FIXED FINAL: All interview subjects population completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ INTERVIEW FIXED FINAL: All interview subjects population failed:', error);
    process.exit(1);
  });
