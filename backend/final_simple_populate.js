...
async function finalSimplePopulate() {
  try {
    console.log('🔍 FINAL SIMPLE: Final simple population...');
    ...
    console.log(`🔍 FINAL SIMPLE: Processing ${students.rowCount} students with ${subjects.rowCount} subjects`);
    ...
    for (const result of interviewResults.rows) {
      ...
      console.log(`🔍 FINAL SIMPLE: Interview result for ${result.admission_number} - Position: ${position}, Grade: ${grade}`);
      position++;
    }
    
    console.log('✅ FINAL SIMPLE: PreFormOne interview data populated successfully!');
    console.log(`🔍 FINAL SIMPLE: Interview results: ${interviewResults.rowCount} students positioned`);
    console.log(`🔍 FINAL SIMPLE: PDF Generation: READY`);
    
  } catch (error) {
    console.error('🔍 FINAL SIMPLE: Error populating data:', error);
    throw error;
  }
}
...
finalSimplePopulate()
  .then(() => {
    console.log('✅ FINAL SIMPLE: Data population completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ FINAL SIMPLE: Data population failed:', error);
    process.exit(1);
  });
